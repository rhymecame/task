...
@implementation WFSearchListCell
...
- (void)layoutSubviews
{
    ...
    [_gridImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [make.left.mas_equalTo(self)setOffset:WFMargin * 2];
        make.size.mas_equalTo(CGSizeMake(90, 90));
    }];
    
    [_autotrophyImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        [make.left.mas_equalTo(_gridImageView.mas_right)setOffset:WFMargin];
    ...
    [_colonButton mas_makeConstraints:^(MASConstraintMaker *make) {
        [make.right.mas_equalTo(self)setOffset:-WFMargin];
        [make.bottom.mas_equalTo(self)setOffset:-WFMargin];
...
- (void)setSearchItem:(WFSearchItem *)searchItem {
    ...
    //首行缩进
    [DCSpeedy dc_setUpLabel:_gridLabel Content:searchItem.name IndentationFortheFirstLineWith:_gridLabel.font.pointSize * 2.5];
}
