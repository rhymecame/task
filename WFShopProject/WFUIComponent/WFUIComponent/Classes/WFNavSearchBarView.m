...
@interface WFNavSearchBarView ()

@end

static bool closeIntrinsic = false;//Intrinsic的影响

@implementation WFNavSearchBarView
...
-(CGSize)intrinsicContentSize
{
    if (closeIntrinsic) {
        ...
        return CGSizeMake(self.wf_width, self.wf_height);
    }
}
...
- (void)setUpUI
{
    ...
    _placeholdLabel.font = [UIFont wf_pfr14];
...
- (void)layoutSubviews
{
    ...
    _placeholdLabel.frame = CGRectMake(WFMargin, 0, self.wf_width - 50, self.wf_height);
    
    [_placeholdLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        [make.left.equalTo(self)setOffset:WFMargin];
        make.top.equalTo(self);
        make.height.equalTo(self);
        
    }];
    [_voiceImageBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        [make.right.equalTo(self)setOffset:-WFMargin];
        make.top.equalTo(self);
        make.height.equalTo(self);
    }];
